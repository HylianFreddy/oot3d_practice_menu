    ...
    {
        {
            ...
            .hex      = false,
            .title    = "Timer 1 State",
            .method   = File_SetTimerStateAndValue,
        },
        {
            .amount   = 0,
            .isSigned = true,
...
static void File_FileMenuInit(void) {
    ...
    FileMenu.items[FILE_ITEM_DROPS].on =
        (gSaveContext.infTable[0x19] & 0x0100) && (gSaveContext.itemGetInf[1] & 0x0008);
    ...
    FileMenu.items[FILE_INTRO_CUTSCENES].on =
        ((gSaveContext.eventChkInf[0xA] & 0x01FB) + (gSaveContext.eventChkInf[0xB] & 0x07FE) +
             (gSaveContext.eventChkInf[0xC] & 0x8081) ==
         0x01FB + 0x07FE + 0x8081);
    FileMenu.items[FILE_BLUE_WARPS_CLEARED].on =
        ((gSaveContext.eventChkInf[0x0] & 0x0080) + (gSaveContext.eventChkInf[0x2] & 0x0020) +
             (gSaveContext.eventChkInf[0x3] & 0x0080) + (gSaveContext.eventChkInf[0x4] & 0x0700) ==
         0x0080 + 0x0020 + 0x0080 + 0x0700);
...
static void File_SetTimerStateAndValue(s32 selected) {
    ...
    s16* varToSet;
    switch (selected) {
        case FILE_TIMER1STATE:
            varToSet = &gSaveContext.timer1State;
            break;
        case FILE_TIMER1VALUE:
            varToSet = &gSaveContext.timer1Value;
            break;
        case FILE_TIMER2STATE:
            varToSet = &gSaveContext.timer2State;
            break;
        case FILE_TIMER2VALUE:
            varToSet = &gSaveContext.timer2Value;
            break;
        default:
            return;
    }
    *(varToSet) = TimersMenu.items[selected].amount;
    ...
    if (selected == FILE_TIMER1STATE) {
        Draw_DrawFormattedString(70, 30 + (FILE_TIMER1STATE % AMOUNT_MENU_MAX_SHOW) * SPACING_Y, COLOR_WHITE,
                                 Timer1States[gSaveContext.timer1State]);
    } else if (selected == FILE_TIMER2STATE) {
        Draw_DrawFormattedString(70, 30 + (FILE_TIMER2STATE % AMOUNT_MENU_MAX_SHOW) * SPACING_Y, COLOR_WHITE,
                                 Timer2States[gSaveContext.timer2State]);
    }
}
