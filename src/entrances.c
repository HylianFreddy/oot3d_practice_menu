...
void EntranceWarp(s16 chosenEntranceIndex, s32 chosenAge, s32 chosenCutsceneIndex, u32 chosenTimeIndex,
                  s32 useFadeOut) {
    ...
    if (chosenCutsceneIndex == -1) {
        // this prevents crashes because warping with CS 0xFFEF ("None") would keep the previous CS number
...
void EntranceSelectMenuShow(EntrancesByScene* entrances, const u8 manualSelection) {
    ...
    do {
        ...
        Draw_DrawFormattedString(30, 30 + SPACING_Y * Entrance_Select_Menu_Time, COLOR_WHITE, "Time of Day: %s",
                                 TimeNames[chosenTime]);
        Draw_DrawCharacter(10, 30 + SPACING_Y * Entrance_Select_Menu_Time, COLOR_TITLE,
                           selected == Entrance_Select_Menu_Time ? '>' : ' ');

        if (cutsceneIndex < -1) {
            cutsceneIndex = -2;
            Draw_DrawString(30, 30 + SPACING_Y * Entrance_Select_Menu_CsIdx,
                            (selected == Entrance_Select_Menu_CsIdx) ? curColor : COLOR_WHITE,
                            "Cutscene Number on Load: Forced Cleared (BiT)");
        } else if (cutsceneIndex == -1) {
            Draw_DrawString(30, 30 + SPACING_Y * Entrance_Select_Menu_CsIdx,
                            (selected == Entrance_Select_Menu_CsIdx) ? curColor : COLOR_WHITE,
                            "Cutscene Number on Load: None                ");
        } else {
            Draw_DrawFormattedString(30, 30 + SPACING_Y * Entrance_Select_Menu_CsIdx,
                                     (selected == Entrance_Select_Menu_CsIdx) ? curColor : COLOR_WHITE,
                                     "Cutscene Number on Load: %02d                  ", cutsceneIndex);
        }
        Draw_DrawCharacter(10, 30 + SPACING_Y * Entrance_Select_Menu_CsIdx, COLOR_TITLE,
                           selected == Entrance_Select_Menu_CsIdx ? '>' : ' ');
        ...
        for (s32 i = 0; i < ENTRANCE_MENU_MAX_SHOW && page * ENTRANCE_MENU_MAX_SHOW + i < entrances->nbItems; ++i) {
            ...
            Draw_DrawString(45, 30 + (Entrance_Select_Menu_Etcs + i) * SPACING_Y, COLOR_WHITE,
                            entrances->items[j].title);
            Draw_DrawCharacter(10, 30 + (Entrance_Select_Menu_Etcs + i) * SPACING_Y, COLOR_TITLE,
                               selected == (Entrance_Select_Menu_Etcs + j) ? '>' : ' ');
        }

        if (manualSelection) {
            Draw_DrawFormattedString(30, 30 + SPACING_Y * Entrance_Select_Menu_Etcs,
                                     selected == Entrance_Select_Menu_Etcs ? curColor : COLOR_WHITE,
                                     "Entrance Index: 0x%04X", chosenEntranceIndex);
            Draw_DrawCharacter(10, 30 + SPACING_Y * Entrance_Select_Menu_Etcs, COLOR_TITLE,
                               selected == Entrance_Select_Menu_Etcs ? '>' : ' ');

            Draw_DrawString(30, 30 + SPACING_Y * Entrance_Select_Menu_Go, COLOR_WHITE, "Go");
            Draw_DrawCharacter(10, 30 + SPACING_Y * Entrance_Select_Menu_Go, COLOR_TITLE,
                               selected == Entrance_Select_Menu_Go ? '>' : ' ');
        }
        ...
        if (chosen) {
            ...
            if (pressed & BUTTON_A) { // select option
                if (manualSelection && selected == Entrance_Select_Menu_Etcs) {
                    ...
                    s16 entranceIndex = manualSelection
                                            ? chosenEntranceIndex
                                            : entrances->items[selected - Entrance_Select_Menu_Etcs].entranceIndex;
        ...
        if (selected < 0) {
            selected =
                (manualSelection ? Entrance_Select_Menu_Go : (Entrance_Select_Menu_Etcs + entrances->nbItems - 1));
        } else if (selected >
                   (manualSelection ? Entrance_Select_Menu_Go : (Entrance_Select_Menu_Etcs + entrances->nbItems - 1))) {
        ...
        if (ADDITIONAL_FLAG_BUTTON) {
            Draw_DrawFormattedStringTop(60, 190, COLOR_WHITE, "Current Entrance: %04X",
                                        (u16)gSaveContext.entranceIndex);
            Draw_DrawFormattedStringTop(60, 200, COLOR_WHITE, "Current Cutscene: %04X",
                                        (u16)gSaveContext.cutsceneIndex);
            Draw_DrawFormattedStringTop(60, 210, COLOR_WHITE, "Next Entrance:    %04X",
                                        (u16)gGlobalContext->nextEntranceIndex);
            Draw_DrawFormattedStringTop(60, 220, COLOR_WHITE, "Next Cutscene:    %04X",
            ...
            //     Draw_DrawFormattedStringTop(60, 220, COLOR_WHITE, "Next Cutscene: %02d                  ",
...
EntrancesByScene Entrances_HyruleCastleGanonsCastleExterior = {
    "Hyrule Castle / Ganon's Castle Exterior",
    .nbItems          = 5,
    .initialCursorPos = 3,
    {
        { 0x0138, "From Market" },
