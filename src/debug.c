...
static void DebugActors_ShowMoreInfo(Actor* actor) {
    ...
    do {
        ...
        Draw_DrawFormattedString(30, 30 + SPACING_Y * lines++, COLOR_WHITE, "Type:            %s",
                                 ActorTypeNames[actor->type]);
        Draw_DrawFormattedString(30, 30 + SPACING_Y * lines++, COLOR_WHITE, "Params:          %04X",
                                 actor->params & 0xFFFF);
        Draw_DrawFormattedString(30, 30 + SPACING_Y * lines++, COLOR_WHITE,
                                 "Pos:             x:%05.2f  y:%05.2f  z:%05.2f", actor->world.pos.x,
                                 actor->world.pos.y, actor->world.pos.z);
        Draw_DrawFormattedString(30, 30 + SPACING_Y * lines++, COLOR_WHITE, "Rot:             x:%04X  y:%04X  z:%04X",
                                 actor->world.rot.x & 0xFFFF, actor->world.rot.y & 0xFFFF, actor->world.rot.z & 0xFFFF);
        Draw_DrawFormattedString(30, 30 + SPACING_Y * lines++, COLOR_WHITE,
                                 "Velocity:        x:%05.2f  y:%05.2f  z:%05.2f", actor->velocity.x, actor->velocity.y,
                                 actor->velocity.z);
        ...
        Draw_DrawFormattedString(30, 30 + SPACING_Y * lines++, COLOR_WHITE, "Dist. from Link: xz:%05.2f  y:%05.2f",
                                 actor->xzDistToPlayer, actor->yDistToPlayer);
        Draw_DrawFormattedString(30, 30 + SPACING_Y * lines++, COLOR_WHITE, "Text ID:         %04X",
                                 actor->textId & 0xFFFF);
...
static bool DebugActors_SpawnActor(void) {
    ...
    do {
        ...
        Draw_DrawFormattedString(200, 70, selected == 2 ? COLOR_GREEN : COLOR_WHITE,
                                 storedPosRotIndex < 0 ? "Position: Link    " : "Position: Stored %01d",
                                 storedPosRotIndex);
        ...
        Draw_DrawString(30, 160, COLOR_TITLE,
                        "A: Edit      X: Fetch Position from Link\n"
        ...
        if (pressed & BUTTON_A) {
            ...
            Menu_EditAmount(xCoords[selected], 70, values[selected], VARTYPE_U16, 0, selected == 2 ? 8 : 0,
                            digitCounts[selected], selected != 2);
            if (selected == 2) {
            ...
            Actor_Spawn(&(gGlobalContext->actorCtx), gGlobalContext, newId, p.pos.x, p.pos.y, p.pos.z, p.rot.x, p.rot.y,
                        p.rot.z, newParams);
            return true;
...
static void DebugActors_ShowActors(void) {
    ...
    do {
        ...
        for (s32 i = 0; i < ACTOR_LIST_MAX_SHOW && page * ACTOR_LIST_MAX_SHOW + i < listSize; ++i) {
            ...
            Draw_DrawFormattedString(30, 30 + (1 + i) * SPACING_Y, curColor, "Addr: %08X  ID: %04X",
                                     actorList[j].instance, actorList[j].id & 0xFFFF);
...
static void Debug_ShowObjects(void) {
    ...
    do {
        Draw_Lock();
        Draw_DrawFormattedString(10, 10, COLOR_TITLE, "Currently Loaded Objects: %02d/%02d",
                                 gGlobalContext->objectCtx.num, OBJECT_EXCHANGE_BANK_MAX);
        Draw_DrawFormattedString(30, 50, COLOR_TITLE, "Object ID: %04X      (Y) Push    (X) Pop", objectId);
        Draw_DrawFormattedString(30 + (14 - digitIdx) * SPACING_X, 50, COLOR_GREEN, "%01X",
                                 (objectId >> (digitIdx * 4)) & 0xF);
...
#define CURSOR_CHAR 176
    ...
    do {
        ...
        for (s32 j = 0; j < 16; j++) {
            Draw_DrawFormattedString(70 + j * 2 * SPACING_X, 50, (row > 0 && column == j) ? COLOR_TITLE : COLOR_GRAY,
                                     "%X", j);
        }
        // Flags row indices
        for (s32 i = 0; i < RowAmounts[group]; i++) {
            Draw_DrawFormattedString(30, 50 + (i + 1) * SPACING_Y, (row == i + 1) ? COLOR_TITLE : COLOR_GRAY, "%04X",
                                     i * 16 + (group == FLAGS_INF_TABLE_2 ? 0x100 : 0));
        }
        // Flags
        for (s32 i = 0; i < RowAmounts[group]; i++) {
            for (s32 j = 0; j < 16; j++) {
                Draw_DrawCharacter(70 + j * 2 * SPACING_X, 50 + (i + 1) * SPACING_Y,
                                   FLAG_STATUS(i, j) ? COLOR_GREEN : COLOR_RED, FLAG_STATUS(i, j) ? '1' : '0');
            }
        }
        // Cursor
        if (row > 0) {
        ...
        if (pressed & BUTTON_B) {
            ...
            if (pressed & PAD_DOWN) {
                row++;
                if (row > RowAmounts[group] && column > 1)
                    column = 1;
            }
        ...
        if (groupToSelect != group) {
            ...
            if (group > 14)
                ...
                group = 14;
            else if (group == FLAGS_UNK1)
                group = FLAGS_CHEST;
            else if (group == FLAGS_UNK2)
                group = FLAGS_TEMP_SWITCH;

            groupToSelect = group;

            switch (group) {
                case FLAGS_GOLD_SKULLTULAS:
                    flags = (u16*)&gSaveContext.gsFlags;
                    break;
                case FLAGS_EVENT_CHK_INF:
                    flags = (u16*)&gSaveContext.eventChkInf;
                    break;
                case FLAGS_ITEM_GET_INF:
                    flags = (u16*)&gSaveContext.itemGetInf;
                    break;
                case FLAGS_INF_TABLE_1:
                    flags = (u16*)&gSaveContext.infTable;
                    break;
                case FLAGS_INF_TABLE_2:
                    flags = ((u16*)&gSaveContext.infTable) + 16;
                    break;
                case FLAGS_EVENT_INF:
                    flags = (u16*)&gSaveContext.eventInf;
                    break;
                default:
                    flags = ((u16*)&(gGlobalContext->actorCtx.flags.swch)) + group * 2;
                    break;
            }
    ...
    {
        {
            ...
            .nDigits  = 4,
            .hex      = true,
            .title    = "Lock=2000, DownA=0020, ReturnA=0010,...",
            .method   = NULL,
        },
        {
            .amount   = 0,
            ...
            .nDigits  = 4,
            .hex      = true,
            .title    = "LedgeCancel=4000, GID=0400, GJ=0800,...",
            .method   = NULL,
        },
        {
            .amount   = 0,
            ...
            .nDigits  = 4,
            .hex      = true,
            .title    = "Invisible=2000, BlankA=0004,...",
            .method   = NULL,
        },
        {
            .amount   = 0,
        ...
        },
        {
            ...
            .nDigits  = 3,
            .hex      = false,
            .title    = "Held Item ID (simulate QuickDraw)",
            .method   = NULL,
        },
    },
};
...
static void Debug_PlayerStatesMenuShow(void) {
    if (isInGame()) {
        ...
        PLAYER->stateFlags1 = (PlayerStatesMenu.items[PLAYERSTATES_PART1].amount << 0x10) |
                              PlayerStatesMenu.items[PLAYERSTATES_PART2].amount;
        PLAYER->stateFlags2 = (PlayerStatesMenu.items[PLAYERSTATES_PART3].amount << 0x10) |
                              PlayerStatesMenu.items[PLAYERSTATES_PART4].amount;
...
#define WHITE_OR_BLUE_AT(X, Y) ((selectedRow == X && selectedColumn == Y) ? COLOR_TITLE : COLOR_WHITE)
    ...
    do {
        ...
        // Address selection
        Draw_DrawFormattedString(30, 30, (selectedRow == 0 && selectedColumn == 0) ? COLOR_GREEN : COLOR_WHITE, "%08X",
                                 memoryEditorAddress);
        // Scroll buttons
        ...
        if (sideInfo == SIDEINFO_TABLE_DATA) {
            ...
            Draw_DrawFormattedString(240, 30 + SPACING_Y * 2, COLOR_GRAY,
                                     "Table Start\n  %08X\nElement Size\n  %04X\nIndex Type\n  %s\nIndex\n %c%04X",
                                     storedTableStart, tableElementSize, TableIndexTypeNames[tableIndexType],
                                     tableIndexSign, tableIndexAbs);
        }
        // Byte index markers
        for (s32 j = 0; j < 8; j++) {
        ...
        for (s32 i = 0; i < 16; i++) {
            ...
            Draw_DrawFormattedString(30, yPos, selectedRow == (i + 2) ? COLOR_TITLE : COLOR_GRAY, "%08X",
                                     memoryEditorAddress + i * 8);
            if (isValidMemory) {
                for (s32 j = 0; j < 8; j++) {
...
static void MemoryEditor_GoToPreset(void) {
    ...
    do {
        ...
        for (s32 j = 0; j < presetCount; j++) {
            Draw_DrawFormattedString(30, 30 + j * SPACING_Y, (selected == j) ? COLOR_TITLE : COLOR_WHITE, "%08X : %s",
                                     presets[j].address, presets[j].name);
...
static void MemoryEditor_FollowPointer(void) {
    ...
    if (pointerAddress >= (u32)gGlobalContext->sceneSegment &&
        pointerAddress <
            (u32)gGlobalContext->sceneSegment + 0x1000) // Manage segment addresses for the scene file headers
...
static void MemoryEditor_TableSettings(void) {
    ...
    do {
        ...
        Draw_DrawFormattedString(30, 120 + SPACING_Y, selected == 0 ? COLOR_GREEN : COLOR_WHITE,
                                 "Table Element Size: 0x%04X", tableElementSize);
        Draw_DrawFormattedString(30, 120 + SPACING_Y * 2, selected == 1 ? COLOR_GREEN : COLOR_WHITE,
                                 "Table Index Type:   < %s>", TableIndexTypeNames[tableIndexType]);
        Draw_DrawFormattedString(30, 120 + SPACING_Y * 3, selected == 2 ? COLOR_GREEN : COLOR_WHITE,
                                 "Table Index:       %c0x%04X", tableIndexSign, tableIndexAbs);

        Draw_DrawFormattedString(30, 120 + SPACING_Y * 6, selected == 3 ? COLOR_GREEN : COLOR_WHITE, "Side Info: %s",
                                 SideInfoOptions[sideInfo]);
        ...
        if (pressed & BUTTON_B) {
            ...
            switch (selected) {
                case 0:
                    Menu_EditAmount(30 + SPACING_X * 19, 120 + SPACING_Y, &tableElementSize, VARTYPE_U16, 0, 0, 4,
                                    true);
                    ...
                    break;
                case 2:
                    Menu_EditAmount(30 + SPACING_X * 19, 120 + SPACING_Y * 3, &tableIndex, tableIndexType, 0, 0, 4,
                                    true);
...
u32 MemoryEditor_GetSelectedByteAddress(void) {
    u32 offset = selectedRow > 1 ? ((selectedRow - 2) * 8) + selectedColumn : 0;
